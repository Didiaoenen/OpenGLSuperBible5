...
GLuint				hdrTextures[3];
GLuint				lutTxtures[1];
GLuint				fboTextures[1];
GLuint				hdrTexturesWidth[3];
GLuint				hdrTexturesHeight[3];
GLuint				curHDRTex;
GLuint				fboName;
GLuint              mapTexProg;
...
const char* hdrTextureFiles[] = { "Cannon.exr", "Ocean.exr", "Tree.exr" };
...
bool LoadOpenEXRImage(const char *fileName, GLint textureName, GLuint &texWidth, GLuint &texHeight);
GLfloat* GetTexOffset();
...
void SetupRC(void)
{
	...
	// Setup HDR texture(s)
	glGenTextures(3, hdrTextures);
	for (int i = 0; i < 3; i++)
	{
		glBindTexture(GL_TEXTURE_2D, hdrTextures[i]);
		LoadOpenEXRImage(hdrTextureFiles[i], hdrTextures[i], hdrTexturesWidth[i], hdrTexturesHeight[i]);
	}
	...
	glUniform2fv(glGetUniformLocation(adaptiveProg, "tc_offset"), 25, GetTexOffset());
...
GLfloat* GetTexOffset()
{
	...
	return (GLfloat*)texCoordOffsets;
}
...
{ 
	...
	{
		curHDRTex--;
		curHDRTex = curHDRTex % 3;
		if (curHDRTex < 0)
			return;

		glUseProgram(curProg);
		if (curProg == adaptiveProg)
		{
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_1D, lutTxtures[0]);

			glUniform2fv(glGetUniformLocation(curProg, "tc_offset"), 25, GetTexOffset());
		}
		GenerateOrtho2DMat(screenWidth, screenHeight, hdrTexturesWidth[curHDRTex], hdrTexturesHeight[curHDRTex]);
		gltGenerateOrtho2DMat(hdrTexturesWidth[curHDRTex], hdrTexturesHeight[curHDRTex], fboOrthoMatrix, fboQuad);
	}
